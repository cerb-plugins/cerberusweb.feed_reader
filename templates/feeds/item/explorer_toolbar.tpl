...
<form action="{devblocks_url}{/devblocks_url}" method="post" id="frmExploreFeedItem">
	...
	{if !$model->is_closed}
		<button type="button" class="status close"><span class="cerb-sprite2 sprite-tick-circle"></span> <label>{'common.close'|devblocks_translate|capitalize}</label></button>
	{else}
	...
	<button type="button" onclick="genericAjaxPopup('peek','c=internal&a=showPeekPopup&context={CerberusContexts::CONTEXT_TASK}&context_id=0&context={'cerberusweb.contexts.feed.item'}&context_id={$model->id}',null,false,'500');"><span class="cerb-sprite2 sprite-gear"></span> {'tasks.add'|devblocks_translate|capitalize}</button>
...
<script type="text/javascript">
	$('#frmExploreFeedItem BUTTON.status').click(function() {
		...
		if($btn.hasClass('close')) {
			...
			$btn.removeClass('close').addClass('reopen').find('span').removeClass('sprite-tick-circle').addClass('sprite-folder_out');
			...
			$btn.removeClass('reopen').addClass('close').find('span').removeClass('sprite-folder_out').addClass('sprite-tick-circle');
